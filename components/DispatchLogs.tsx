...
export const DispatchLogs: React.FC = () => {
  ...
  const {
    ...
    totalItems,
    itemsPerPage,
  } = useTableManager<DispatchLog>({
      itemsPerPage: 10,
      initialSortKey: 'timestamp',
      searchableKeys: ['status', 'rule_name', 'target_url', 'status_code'],
  });
  ...
  const renderSortIcon = (key: keyof DispatchLog) => {
  ...
  const tableHeaders: { key: keyof DispatchLog; label: string }[] = [
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-white mb-2">Dispatch Logs</h1>
      <TableControls
          filter={filter}
